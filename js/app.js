...
async function sendOrder(){const btn=$('#send');try{const name=$('#name').value.trim(),phone=digits($('#phone').value),payment=$('#payment').value,email=String($('#email')?.value||'').trim();if(name.length<2)throw new Error('Informe seu nome.');if(phone.length<10)throw new Error('Informe um WhatsApp válido.');if(payment==='Pix'&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))throw new Error('Informe um e-mail válido para gerar o Pix.');saveCustomer();const cashReq=Number(String($('#cashbackUse')?.value||'0').replace(',','.'))||0,payload={client_request_id:crypto.randomUUID(),customer:{name,phone},type:orderType,payment,change_for:$('#change').value.trim(),notes:$('#notes').value.trim(),coupon_code:$('#coupon').value.trim(),cashback_to_use:cashReq,source:'client-v2',items:cart.map(x=>({product_id:x.product.id,qty:x.qty,addon_ids:x.addons.map(a=>a.id),note:x.note}))};if(orderType==='delivery'){payload.address={street:$('#street').value.trim(),number:$('#number').value.trim(),neighborhood_id:$('#hood').value,complement:$('#complement').value.trim(),reference:$('#reference').value.trim()};if(!payload.address.street||!payload.address.number||!payload.address.neighborhood_id)throw new Error('Preencha rua, número e bairro.')}btn.disabled=true;btn.textContent='Enviando…';const j=await post('create-order',payload);localStorage.setItem('caseirao_last_tracking',JSON.stringify({tracking_code:j.tracking_code,phone}));if(payment==='Pix'){btn.textContent='Gerando Pix…';const pix=await post('mercadopago-create-pix',{order_id:j.id,email});cart=[];updateCart();openPixPayment(j,pix,phone);return}cart=[];updateCart();modal(`<div class="head"><h2>Pedido recebido ✅</h2><button class="x" data-close>×</button></div><div class="success">Pedido <b>#${j.order_number}</b><br>Total: <b>${money(j.total)}</b><br>Previsão: <b>${esc(j.eta_text||'a confirmar')}</b></div><button id="trackNow" class="primary">Acompanhar pedido</button>`);$('#trackNow').onclick=()=>openTracking(j.tracking_code,phone)}catch(e){alert(e.message||String(e));if(btn){btn.disabled=false;btn.textContent='Confirmar e enviar'}}}
...
function acceptedOrder(order,phone){
  ...
  modal(`<div class="head"><h2>Pedido recebido ✅</h2><button class="x" data-close>×</button></div><div class="success">Pedido <b>#${order.order_number}</b><br>Total: <b>${money(order.total)}</b><br>Previsão: <b>${esc(order.eta_text||'a confirmar')}</b></div><button id="trackNow" class="primary">Acompanhar pedido</button>`);
...
async function sendOrderSafe(){
  ...
  try{
    ...
    saveCustomer();
    const cashReq=Number(String($('#cashbackUse')?.value||'0').replace(',','.'))||0;
    ...
    if(!state){
      const payload={client_request_id:crypto.randomUUID(),customer:{name,phone},type:orderType,payment,change_for:$('#change').value.trim(),notes:$('#notes').value.trim(),coupon_code:$('#coupon').value.trim(),cashback_to_use:cashReq,source:'client-v2',items:cart.map(x=>({product_id:x.product.id,qty:x.qty,addon_ids:x.addons.map(a=>a.id),note:x.note}))};
    ...
    if(state?.order){
      modal(`<div class="head"><h2>Pedido já registrado</h2><button class="x" data-close>×</button></div><div class="notice">O pedido <b>#${esc(state.order.order_number)}</b> está salvo. Não envie outro pedido.</div>${state.payment==='Pix'?'<button id="retryPix" class="primary">Gerar Pix novamente</button>':''}<button id="trackSaved" class="secondary">Acompanhar pedido</button>`);
